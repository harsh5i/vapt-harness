...
| AST walker | partial | source-reading AST classifier with intra-function, same-file inter-procedural, same-class self.method/self.attr, **and cross-file** taint flow (T4.3 + T4.6 + T4.7 + T4.8); 5/5 seeded fixtures + 30 unit tests under `tests/test_ast_taint_flow.py` (12 intra-function, 6 cross-function, 6 self/class, 6 cross-file) | `python3 vapt/harness/harness.py source-probe --local-path vapt/harness/fixtures/seeded_bugs_repo` (expects `finding_count=5`) | Taint propagates through Assign / AnnAssign / AugAssign / tuple-unpack within a function, across same-file calls, across methods of the same class, AND across files in the same package via import-resolved callee lookup (`from m import f; f(x)`, `from m import f as g; g(x)`, `import m; m.f(x)`, plus relative `from .sibling import f`). The walker now runs at package granularity: parses all files, builds a global symbol table of module-level free functions, resolves each file's import aliases against it, then runs the fixed-point over the union of all functions. Stdlib / third-party imports never resolve, so taint cannot leak through them. Still does **not** resolve attribute taint on objects other than `self`, model container aliasing, or follow chained-attribute calls (`self.helper().bar()`). Real-target validation (≥1 small OSS Python project with a known logic flaw) still pending. |
