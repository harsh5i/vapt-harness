...
| AST walker | partial | source-reading AST classifier with intra-function + same-file inter-procedural taint flow (T4.3 + T4.6); 5/5 seeded fixtures + 18 unit tests under `tests/test_ast_taint_flow.py` (including 6 cross-function cases: positional arg, literal-only call, tainted return, keyword arg, cross-function SQL, mutual recursion termination) | `python3 vapt/harness/harness.py source-probe --local-path vapt/harness/fixtures/seeded_bugs_repo` (expects `finding_count=5`) | Taint propagates through Assign / AnnAssign / AugAssign / tuple-unpack within a function AND across same-file calls via fixed-point propagation (positional-by-index, keyword-by-name, tainted-return into call-site assignments; bounded by max 6 iterations for mutual recursion). Still does **not** cross file boundaries, model attribute taint, resolve method dispatch through `self`, or unwind aliasing through containers. Real-target validation (≥1 small OSS Python project with a known logic flaw) still pending. |
