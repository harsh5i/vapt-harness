...
| Reference probe: supply_chain_diff | implemented | `vapt/harness/source/supply_chain_diff.py` + `vapt/harness/probes/supply_chain_diff.py`; diffs a release tarball against a corresponding git checkout and emits per-file findings (`tarball_only`, `content_differs`, `git_only`). Archive support for `.tar.gz`/`.tgz`/`.tar.bz2`/`.tar.xz`/`.tar`/`.zip` with path-traversal-safe extraction. Severity HIGH for code-suffix files (.js/.py/.rb/.go/.c/.sh/.m4/.so/.dll/...), MEDIUM for docs/configs, LOW for git_only. Built-in suppression for autotool-generated paths (`configure`, `Makefile.in`, `aclocal.m4`, ...) and walk-skip for `.git/` / `node_modules/`. 10 unit tests under `tests/test_supply_chain_diff.py`. Spot-validated against `is-number@7.0.0` (npm pack + git tag): 0 tarball_only / 0 content_differs (clean release baseline), 11 git_only LOW (expected `.npmignore` exclusions). | `./.venv-vapt/bin/python -m pytest vapt/harness/tests/test_supply_chain_diff.py` ; `python3 vapt/harness/harness.py probes-run --probe supply_chain_diff --knob tarball_root=... --knob git_root=...` | Auto-fetch (`npm pack` / `git clone --branch`) + `.npmignore` parsing deferred (see `knowledge/lessons/2026-06-03_supply_chain_diff.md`). |
| Reference probe: header_trust | implemented | `vapt/harness/source/header_trust.py` + `vapt/harness/probes/header_trust.py`; static scanner over Ruby/Python/JS/TS source that flags reads of HTTP headers known to be spoofable across reverse-proxy / framework setups. 36-header registry severity-tagged via case-study evidence (HIGH for `x-middleware-subrequest`, `x-original-url`, `x-rewrite-url`, `x-original-host`, `x-forwarded-host`, `x-impersonate-user`, `x-tenant-id`, etc.; MEDIUM for x-forwarded-for/proto/host, x-real-ip, x-http-method-override, _method, cf-connecting-ip, true-client-ip). Header-name normalisation (`HTTP_X_FORWARDED_FOR` -> `x-forwarded-for`). 14 unit tests under `tests/test_header_trust.py`. Both watch-profile targets clean (Discourse uses `action_dispatch.remote_ip` abstraction, GitLab reads only custom `X-Gitlab-*` headers); intended yield surface is Next.js / Node / Express SaaS that reads headers directly. | `./.venv-vapt/bin/python -m pytest vapt/harness/tests/test_header_trust.py` | Trust-proxy config-file audit + effect-side taint chaining deferred (see `knowledge/lessons/2026-06-03_header_trust.md`). |
| Reference probe: js_dep_audit | implemented | `vapt/harness/source/js_deps.py` (LockfileParser + DependencyAuditor) + `vapt/harness/probes/js_dep_audit.py` + `scripts/osv_populate.py` (OSV.dev `/v1/querybatch` populator). Parses `package-lock.json` (v1 nested, v2/v3 flat), `yarn.lock` v1, and `pnpm-lock.yaml` v6+ without a YAML dependency. Cross-references against the local OSV cache (offline-safe; reuses `gates/osv.py` schema). 13 unit tests under `tests/test_js_deps.py`. **First substantial outcome on watch-profile targets: 71 vulnerable deps across Discourse (15) and GitLab (56) in 0.3s after a one-time cache populate (~30s for 3498 deps)**, including a malicious-package lead (`MAL-2025-257 -> vue-loader-vue3 17.4.2` in GitLab). | `./.venv-vapt/bin/python -m pytest vapt/harness/tests/test_js_deps.py` ; `python3 scripts/osv_populate.py <target_root>` then probe via `harness.py probes-run --probe js_dep_audit ...` | Severity/CVSS enrichment deferred to a per-ID `/v1/vulns/{id}` follow-up pass (see `knowledge/lessons/2026-06-03_js_dep_audit.md`). |
| Reference probe: postmessage_origin | implemented | `vapt/harness/source/postmessage.py` + `vapt/harness/probes/postmessage_origin.py`; static walker locates `addEventListener("message", ...)` and `*.onmessage = ...` registrations, captures the handler body via brace-balance, follows named-handler references in-file, and classifies origin checks (strong: `===` / `Array.includes(event.origin)` / `arr.indexOf(event.origin) !== -1`; weak: indexOf/endsWith/startsWith/.includes on origin / regex .test on origin; missing: no `.origin` reference). Built-in suppression for same-origin channels (ServiceWorker, MessageChannel ports, BroadcastChannel, Worker). 16 unit tests under `tests/test_postmessage_walker.py`. Validated against Discourse: 5 raw -> 0 findings after same-origin filter + named-handler resolution; all 5 handlers correctly secured. | `./.venv-vapt/bin/python -m pytest vapt/harness/tests/test_postmessage_walker.py` | Cross-file named-handler resolution and effect-severity scoring deferred (see `knowledge/lessons/2026-06-03_postmessage_walker.md`). |
| Reference probe: js_bundle_analyzer | implemented | `vapt/harness/source/js_bundle.py` + `vapt/harness/probes/js_bundle_analyzer.py`; static scanner emitting `endpoint` / `admin_route` / `secret` findings with built-in placeholder + test-path negative controls and a minified-file guard backed by a non-backtracking string-literal regex (unrolled-loop form). 12 unit tests under `tests/test_js_bundle_analyzer.py`. Validated end-to-end against Discourse frontend + plugins + app (~2900 JS/TS files) in 1.2s wall clock: 398 findings (388 admin_route, 10 endpoint, 0 secret); all 12 cross-context admin-route candidates triaged as correctly authz-gated server-side. | `./.venv-vapt/bin/python -m pytest vapt/harness/tests/test_js_bundle_analyzer.py` | Ember route-map walker and source-map chasing deferred until a target needs them (see `knowledge/lessons/2026-06-03_js_bundle_analyzer.md`). |
...
| Unit tests | implemented | 199 tests green across 15 suites (+3 opt-in real-world smoke tests skipped by default): `test_ast_taint_flow.py` (36, incl. cross-function + self/class + cross-file + non-self attr/container), `test_authorization_scope.py` (13), `test_cold_start_commands.py` (16), `test_dedup_novelty.py` (6), `test_gates_promotion.py` (17), `test_imports.py` (4), `test_header_trust.py` (14), `test_io_atomic.py` (10), `test_js_bundle_analyzer.py` (12), `test_js_deps.py` (13), `test_outcome_ingest.py` (13), `test_outcome_tuning.py` (9), `test_postmessage_walker.py` (16), `test_realworld_smoke.py` (3, opt-in), `test_supply_chain_diff.py` (10), `test_validators.py` (10) | `./.venv-vapt/bin/python -m pytest vapt/harness/tests/` ; `VAPT_REALWORLD=1 ./.venv-vapt/bin/python -m pytest vapt/harness/tests/test_realworld_smoke.py` | T3.1 acceptance met. Per-engagement integration tests still future. |
